...
#define ESP_ADJUST2 0
    template<int BITS,int PX> __attribute__ ((always_inline)) inline static void writeBits(register uint32_t & last_mark, register Lines & b, PixelController<RGB_ORDER, LANES, PORT_MASK> &pixels) { // , register uint32_t & b2)  {
        Lines b2=b ;
        //Lines b4=b3;
        transpose32x1_noinline(b.bytes,b2.shorts);
        ...
        for(register uint32_t i = 0; i < 8; i++) { //USED_LANES
            ...
            *FastPin<FIRST_PIN>::cport() = nword;
            ...
            if(16+i<LANES)
                b.bytes[16+i] = pixels.template loadAndScale<PX>(pixels,i,d,scale);
    ...
    // gcc will use register Y for the this pointer.ICACHE_RAM_ATTR
    static uint32_t ICACHE_RAM_ATTR showRGBInternal(PixelController<RGB_ORDER, LANES, PORT_MASK> &allpixels) {
        ...
        */
        ...
        while(allpixels.has(1)) {
            ...
            writeBits<17+XTRA0,1>(last_mark, b0,allpixels);
            //last_mark = __clock_cycles();
            // Write second byte, read 3rd byte
            writeBits<17+XTRA0,2>(last_mark, b0, allpixels);
            //last_mark = __clock_cycles();
            allpixels.advanceData();
            
            // Write third byte
            writeBits<17+XTRA0,0>(last_mark, b0,allpixels);
        ...
        for(int i=0;i<32;i++)
        {
            ...
            if( (porti & 0x1)>0 )
            {
               /* pinMode(i,INPUT);
                ...
                pinMode(i,INPUT);
               // gpio_pulldown_en((gpio_num_t)i );
                rtc_gpio_set_direction((gpio_num_t)i,RTC_GPIO_MODE_DISABLED);
                //esp_err_tgpio_reset_pin((gpio_num_t)i );
                //gpio_set_direction((gpio_num_t)i, GPIO_MODE_OUTPUT_OD );
                //pinMode(i,OUTPUT);
