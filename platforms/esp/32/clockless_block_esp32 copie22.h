...
#define FIX_BITS2(bits)  (  ((bits & 0xFF00L)<<4) | ((bits & 0xE0L) << 16) |  ((bits & 0x1EL)<<1)  | (bits & 0x1L)   )
...
#define LAST_PIN 35
#define PORT_MASK_TOTAL  ( (PORT_MASK << REAL_FIRST_PIN ) | PORT_MASK2  )
...
#define ESP_ADJUST2 0
	...
	for(register uint32_t i = 0; i < 8; i++) { //USED_LANES
        ...
        *FastPin<FIRST_PIN>::sport() = PORT_MASK_TOTAL ;
      ...
      // uint32_t nword =   (  ((uint32_t)(~b2.bytes[7-i]) & PORT_MASK) << REAL_FIRST_PIN ) | (FIX_BITS(  (uint32_t)(~b4.bytes[7-i]) ) & PORT_MASK2) ; //((uint32_t)(~b4.bytes[7-i]) & PORT_MASK2) << 21 );//
        ...
        uint32_t nword = FIX_BITS2 ((uint32_t)(~b2.shorts[7-i]))  & PORT_MASK_TOTAL;
        
        while((__clock_cycles() - last_mark) < (T1-6));
	    *FastPin<FIRST_PIN>::cport() = nword;
	    
	    while((__clock_cycles() - last_mark) < (T1+T2));
        *FastPin<FIRST_PIN>::cport() = PORT_MASK_TOTAL;
