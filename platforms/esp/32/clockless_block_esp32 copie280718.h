...
//ert=(uint32_t)(~b2.shorts[7-i]);
  ...
  // __asm__ __volatile__("nop;nop;nop;nop;nop;nop;nop;");
	   ...
	   *FastPin<FIRST_PIN>::cport() = nword;
	   // GPIO.out=GPIO.out  ^ nword;
        //GPIO.out=saveg  | nword;
       ...
       // GPIO.out_w1tc = nword;
        ...
        //GPIO.out=(GPIO.out  | PORT_MASK) ^ PORT_MASK;
       //GPIO.out=saveg;
...
static uint32_t ICACHE_RAM_ATTR showRGBInternal(PixelController<RGB_ORDER, LANES, PORT_MASK> &allpixels) {
    ...
    io_conf.pin_bit_mask = PORT_MASK;
    ...
    //bit mask of the pins that you want to set,e.g.GPIO18/19
    io_conf2.pin_bit_mask = PORT_MASK;
    //disable pull-down mode
    io_conf2.pull_down_en = (gpio_pulldown_t)1;
    //disable pull-up mode
    ...
    //GPIO.enable=GPIO.enable | PORT_MASK;
	 //ets_intr_lock();
